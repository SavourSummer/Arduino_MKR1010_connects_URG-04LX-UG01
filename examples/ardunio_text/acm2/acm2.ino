...
void loop()
{
  ...
  while(client.connected() || client.available())//修改了Serialbug.available为client.available()
  {
    //if(client.available())
    //{
    //String line = client.readStringUntil('\0');
    
    //client.read(data,strlen(data))先完成发送命令PP
    //client.write(line.c_str());//返回命令
    usb_send("PP\n",3);//写入命令
...
do {
    usblen = GD_SEGMENT_LEN;
    usb_receive(hokuyo_data, (uint16_t*)300);
    delay(40);
    if (usblen == 0) {
      continue;
    } else if (usblen < 3000) {
