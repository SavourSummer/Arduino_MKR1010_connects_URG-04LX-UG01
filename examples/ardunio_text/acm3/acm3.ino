...
#endif
uint16_t usblen;
//char* hokuyo_data = (char*) malloc(128);
char* hokuyo_dataa = (char*) malloc(128);
static uint8_t data[24][26];
...
void loop()
{
  ...
  while (client.available() || Serial.available()) //若是接受到服务端发送的命令，当然也可以一个一个字符接受然后发送，雷达可识别
  {

    String line = client.readString();
    client.flush();
    client.write(line.c_str());
    usb_send(line.c_str(), line.length());//已经成功实现服务端发送命令
    ...
    /*if(Serial.available())
      //usb_receive(hokuyo_data, (uint16_t*)2200);
      client.write(hokuyo_data, 2200);*/
    //Serial.flush();
    //delay(10);
    //client.flush();
    /*
